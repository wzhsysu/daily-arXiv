...
TL;DR: Novel robustness losses during downstream training reduce sensitivity to technical variability in histopathology foundation models, improving both robustness and accuracy without retraining foundation models.
...
Method: Introducing novel robustness losses during training of downstream task-specific models from foundation model features, tested with comprehensive experimentation on 27,042 WSIs from 6,155 patients using eight popular foundation models.
...
TL;DR: MNAS-Unet combines Monte Carlo Tree Search with Neural Architecture Search for efficient medical image segmentation, achieving better accuracy with 54% reduced search budget and lightweight 0.6M parameter model.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency and practicality of neural architecture search for medical image segmentation by addressing the high computational cost and resource requirements of existing NAS methods.
...
Result: Outperforms NAS-Unet and other state-of-the-art models on PROMISE12, Ultrasound Nerve, and CHAOS datasets; reduces architecture search budget by 54% (139 vs 300 epochs); achieves lightweight 0.6M parameter model with lower GPU memory consumption.

Conclusion: MNAS-Unet improves search efficiency while maintaining competitive segmentation accuracy under practical resource constraints, enhancing the practical applicability of NAS for medical image segmentation.
...
TL;DR: AeroDGS: Physics-guided 4D Gaussian splatting framework for monocular UAV videos that addresses depth ambiguity and motion estimation challenges in dynamic aerial reconstruction through geometry lifting and physics-guided optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 4D scene reconstruction methods struggle with monocular aerial conditions characterized by single-view capture, wide spatial range, dynamic objects with limited spatial footprint and large motion disparity, leading to severe depth ambiguity and unstable motion estimation.
...
Result: Experiments on synthetic and real UAV scenes demonstrate that AeroDGS outperforms state-of-the-art methods in dynamic aerial reconstruction. The authors also built a real-world UAV dataset spanning various altitudes and motion conditions for evaluation.

Conclusion: AeroDGS effectively addresses the ill-posed nature of monocular aerial reconstruction by combining geometry lifting with physics-guided priors, achieving superior reconstruction fidelity in dynamic aerial environments compared to existing approaches.
...
TL;DR: Deep learning framework using Organ Focused Attention loss eliminates need for manual segmentation in renal tumor malignancy prediction from 3D CT images, achieving competitive performance without segmentation-based cropping.


<details>
  <summary>Details</summary>
Motivation: Existing imaging modalities lack accuracy for reliable malignancy prediction before surgery. Traditional deep learning approaches require manual segmentation of renal tumors from 3D CT images, which is labor-intensive, costly, and expert-dependent, creating a need for segmentation-free prediction methods.

Method: Developed a deep learning framework with Organ Focused Attention (OFA) loss function that modifies attention mechanisms so that organ patches attend only to other organ patches, eliminating the need for segmentation at deployment time for malignancy prediction.

Result: Achieved AUC of 0.685 and F1-score of 0.872 on private UF IDR dataset, and AUC of 0.760 and F1-score of 0.852 on public KiTS21 dataset. These results surpass conventional segmentation-based cropping models, demonstrating improved predictive accuracy without explicit segmentation input.

Conclusion: The OFA-based framework provides a more efficient and reliable method for renal tumor malignancy prediction by eliminating the need for manual segmentation, thereby enhancing clinical decision-making in renal cancer diagnosis while maintaining competitive performance.
...
TL;DR: ViTs exhibit artifacts from lazy aggregation behavior where they use irrelevant background patches as shortcuts for global semantics; proposed solution selectively integrates patch features into CLS token to reduce background influence and improve performance across 12 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers (ViTs) show artifacts across different supervision paradigms and downstream tasks, but the fundamental mechanisms behind these artifacts remain insufficiently elucidated. The paper aims to systematically analyze and address these artifacts.

Method: Through systematic analysis, the authors identify that artifacts originate from lazy aggregation behavior where ViTs use semantically irrelevant background patches as shortcuts to represent global semantics, driven by global attention and coarse-grained semantic supervision. The proposed solution selectively integrates patch features into the CLS token to reduce the influence of background-dominated shortcuts.

Result: The method consistently improves performance across 12 benchmarks under label-, text-, and self-supervision paradigms, demonstrating effectiveness in reducing artifact influence.

Conclusion: The work provides a new perspective on ViT behavior by identifying and addressing lazy aggregation artifacts, offering insights into how ViTs process visual information and suggesting improvements for more robust representations.
...
TL;DR: DeBias-CLIP addresses the bias in CLIP training where long captions typically start with a summary sentence, causing the model to focus on early tokens and weaken alignment with detailed descriptions. The method removes summary sentences during training and uses sentence sub-sampling and token padding to distribute supervision across all caption tokens.


<details>
  <summary>Details</summary>
Motivation: CLIP models are biased toward encoding simple descriptions of salient objects due to training on short captions, leading to coarse alignment on complex scenes. While fine-tuning on long-caption datasets helps, these captions typically begin with a summary sentence that acts as a training shortcut, causing the model to concentrate attention on early tokens and weaken alignment with detailed descriptions.

Method: DeBias-CLIP removes the summary sentence from long captions during training and applies two techniques: sentence sub-sampling to randomly select sentences for training, and text token padding to ensure all token positions receive supervision. This distributes attention across the entire caption without adding trainable parameters.

Result: DeBias-CLIP achieves state-of-the-art performance on long-text retrieval, improves short-text retrieval, and shows reduced sensitivity to sentence order permutations. It serves as a drop-in replacement for Long-CLIP with no additional parameters.

Conclusion: The summary-first structure of long captions creates a training bias in CLIP models. By removing summary sentences and distributing supervision across all tokens, DeBias-CLIP improves alignment with detailed descriptions while maintaining or enhancing retrieval performance across both long and short text scenarios.
...
TL;DR: SimpleOCR addresses modality laziness in MLLMs by forcing visual text reading through visualized questions and randomized styles, achieving robust gains with extreme data efficiency.


<details>
  <summary>Details</summary>
Motivation: To diagnose whether MLLMs genuinely read text in images or rely on parametric shortcuts in text prompts, revealing a capability-utilization gap where models with strong OCR capabilities underperform when text queries are rendered directly onto images.

Method: Introduces Visualized-Question (VQ) setting where text queries are rendered onto images, and SimpleOCR training strategy that transforms training samples into VQ format with randomized styles to invalidate text-based shortcuts and optimize visual text extraction pathways.

Result: SimpleOCR yields robust gains without architectural modifications: surpasses base model by 5.4% and GRPO by 2.7% on four OOD benchmarks, achieves superior performance with 30x fewer samples (8.5K) than RL-based methods, and integrates seamlessly with advanced RL strategies like NoisyRollout.

Conclusion: SimpleOCR effectively addresses modality laziness in MLLMs by forcing visual engagement through structural constraints, demonstrating that plug-and-play training strategies can significantly improve visual grounding without model architecture changes.
...
<details>
  <summary>Details</summary>
Motivation: Cloud offloading for MLLM-based episodic memory QA raises privacy and latency concerns for wearable assistants, motivating investigation of edge implementations to enable real-time, privacy-preserving memory retrieval.

Method: Two-thread asynchronous pipeline with streaming constraints: Descriptor Thread continuously converts video to lightweight textual memory; QA Thread reasons over textual memory to answer queries. Evaluated on QAEgo4D-Closed benchmark with strict resource boundaries.

Result: Consumer-grade 8GB GPU: 51.76% accuracy with 0.41s TTFT; enterprise-grade server: 54.40% accuracy with 0.88s TTFT. Cloud baseline: 56.00% accuracy. Edge solutions achieve competitive performance while preserving privacy.

Conclusion: Edge-based MLLMs offer viable alternative to cloud solutions for episodic memory QA, achieving competitive accuracy with low latency while addressing privacy concerns, enabling practical deployment for wearable assistants.
...
TL;DR: MammoWise is a local multi-model pipeline that transforms open-source VLMs into mammogram report generators and multi-task classifiers, offering privacy, reproducibility, and adaptability through support for various prompting strategies and optional multimodal RAG.


<details>
  <summary>Details</summary>
Motivation: Screening mammography is high-volume, time-sensitive, and documentation-heavy, requiring radiologists to translate subtle visual findings into consistent BI-RADS assessments, breast density categories, and structured narrative reports. Existing VLM-based solutions often rely on closed cloud systems or tightly coupled architectures that limit privacy, reproducibility, and adaptability.

Method: MammoWise is a local multi-model pipeline supporting any Ollama-hosted VLM and mammography dataset. It enables zero-shot, few-shot, and Chain-of-Thought prompting, with optional multimodal Retrieval Augmented Generation (RAG) using a vector database for case-specific context. The system was evaluated on MedGemma, LLaVA-Med, and Qwen2.5-VL using VinDr-Mammo and DMID datasets, assessing report quality (BERTScore, ROUGE-L), BI-RADS classification, breast density, and key findings. Parameter-efficient fine-tuning (QLoRA) was applied to improve reliability.

Result: Report generation was consistently strong and improved with few-shot prompting and RAG. Classification was feasible but sensitive to model and dataset choice. QLoRA fine-tuning of MedGemma improved reliability significantly, achieving BI-RADS accuracy of 0.7545, density accuracy of 0.8840, and calcification accuracy of 0.9341 while preserving report quality.

Conclusion: MammoWise provides a practical and extensible framework for deploying local VLMs for mammography reporting within a unified and reproducible workflow, addressing privacy concerns while maintaining performance through flexible prompting strategies and parameter-efficient fine-tuning.
...
TL;DR: Spatial Credit Redistribution (SCR) is a training-free inference-time intervention that reduces object hallucination in vision-language models by redistributing activation credit from high-attention patches to their context, guided by low-entropy inputs.


<details>
  <summary>Details</summary>
Motivation: Vision-language models frequently hallucinate objects absent from input images due to spatial credit collapse - activation credit concentrating on sparse visual patches in early transformer layers, which suppresses contextual evidence and increases reliance on language priors.
...
Result: SCR reduces hallucination by ~4.7-6.0 percentage points on POPE-Adversarial, cuts CHAIR-s by 3.7-5.2 percentage points (42-51% relative), and CHAIR-i by 2.7-4.4 percentage points (44-58% relative), while preserving CIDEr within 0.8 percentage points. It incurs only 43-56 ms overhead, Pareto-dominating OPERA, VCD, and OVCD.

Conclusion: SCR effectively mitigates object hallucination in VLMs by addressing spatial credit collapse through attention-guided activation redistribution, offering practical real-time deployment with minimal computational overhead while maintaining generation quality.
...
<details>
  <summary>Details</summary>
Motivation: Classic CIR approaches using supervised triplets lose fine-grained cues, while zero-shot methods that caption reference images and merge with edits miss implicit user intent and return repetitive results.

Method: Pix2Key represents queries and candidates as open-vocabulary visual dictionaries, enabling intent-aware constraint matching and diversity-aware reranking in unified embedding space. Includes V-Dict-AE for self-supervised pretraining using only images to improve dictionary representation without CIR-specific supervision.

Result: On DFMM-Compose benchmark, Pix2Key improves Recall@10 up to 3.2 points. Adding V-Dict-AE yields additional 2.3-point gain while improving intent consistency and maintaining high list diversity.

Conclusion: Visual dictionary representation with self-supervised pretraining effectively addresses limitations of existing CIR methods by capturing fine-grained attributes, understanding implicit user intent, and maintaining result diversity.
...
<details>
  <summary>Details</summary>
Motivation: Tau-PET is an important in vivo marker for Alzheimer's disease pathology but is costly and has limited availability, creating a need for MRI-based alternatives that can provide similar diagnostic information.
...
Result: DisQ-HNet maintains reconstruction fidelity and better preserves disease-relevant signal for downstream AD tasks compared to multiple baselines (VAE, VQ-VAE, and UNet), including Braak staging, tau localization, and classification. PID-based Shapley analysis provides modality-specific attribution of synthesized uptake patterns.

Conclusion: DisQ-HNet provides an effective MRI-based alternative to tau-PET imaging by synthesizing tau-PET from structural MRI while offering interpretable insights into how each MRI modality contributes to the prediction through information decomposition and attribution analysis.
...
TL;DR: DSKD framework integrates lexical sense knowledge into decoder LLM training via knowledge distillation, improving semantic understanding without dictionary lookup at inference.


<details>
  <summary>Details</summary>
Motivation: LLMs capture rich contextual embeddings but often lack structured lexical knowledge (word senses, relationships). While sense dictionaries improve encoder models, applying them to decoder generative models remains challenging.

Method: Decoder-based Sense Knowledge Distillation (DSKD) framework integrates lexical resources into decoder-style LLM training through knowledge distillation, eliminating need for dictionary lookup during inference.
...
Conclusion: DSKD successfully bridges the gap between contextual embeddings and structured lexical knowledge for decoder LLMs, improving semantic understanding without compromising inference efficiency.
...
TL;DR: This paper tests LLMs' ability to support interpretative citation context analysis through thick, text-grounded readings of a single case, using prompt-sensitivity analysis to examine how scaffolding and framing affect interpretative outputs.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can support interpretative citation context analysis (CCA) by providing detailed, text-grounded readings rather than just typological classification, and to examine how prompt design systematically influences the interpretative outputs generated by LLMs.

Method: Two-stage GPT-5 pipeline: 1) citation-text-only surface classification and expectation pass, 2) cross-document interpretative reconstruction using full texts of citing and cited documents. Used footnote 6 in Chubin and Moitra (1975) and Gilbert's (1977) reconstruction as a probe. Implemented balanced 2x3 design varying prompt scaffolding and framing. Generated 90 reconstructions yielding 450 distinct hypotheses, then conducted close reading and inductive coding to identify 21 recurring interpretative moves, with linear probability models estimating prompt effects.

Result: GPT-5's surface classification was highly stable, consistently labeling the citation as "supplementary." In reconstruction, the model generated a structured space of plausible alternatives, but prompt scaffolding and examples redistributed attention and vocabulary, sometimes toward strained readings. Compared to Gilbert's analysis, GPT-5 detected the same textual hinges but more often resolved them as lineage and positioning rather than admonishment.

Conclusion: LLMs can serve as guided co-analysts for inspectable, contestable interpretative CCA, but prompt scaffolding and framing systematically influence which plausible readings and vocabularies the model foregrounds, creating both opportunities and risks for interpretative analysis.
...
TL;DR: First Bengali meme dataset (Bn-HIB) with 3,247 memes annotated as Benign, Hate, or Inflammatory, plus MCFM model using co-attention for multimodal fusion, outperforming SOTA on nuanced Bengali meme classification.


<details>
  <summary>Details</summary>
Motivation: Bengali memes can spread offensive/harmful content but detection is challenging due to satire, subtlety, cultural specificity, and low-resource language status; existing research focuses on high-resource languages, creating a critical gap.

Method: Created Bn-HIB dataset (3,247 manually annotated Bengali memes: Benign/Hate/Inflammatory) and proposed MCFM (Multi-Modal Co-Attention Fusion Model) that uses co-attention to identify and fuse critical visual/textual features for classification.

Result: MCFM significantly outperforms several state-of-the-art models on Bn-HIB dataset, demonstrating effectiveness in nuanced Bengali meme classification; Bn-HIB is first to distinguish inflammatory from hate speech in Bengali memes.

Conclusion: Addresses critical research gap for low-resource Bengali language meme analysis; Bn-HIB enables nuanced classification and MCFM's co-attention effectively fuses multimodal features for improved offensive content detection in culturally specific contexts.
...
TL;DR: A multilingual stereotype dataset covering four underrepresented Sub-Saharan African countries (Ghana, Kenya, Nigeria, South Africa) created using community-engaged methods to address global coverage gaps in AI safety evaluation.


<details>
  <summary>Details</summary>
Motivation: Current stereotype repositories lack adequate global coverage, particularly for underrepresented regions like Sub-Saharan Africa, limiting comprehensive assessment of generative AI model safety and fairness.

Method: Socioculturally-situated, community-engaged approach using telephonic surveys moderated in native languages, with deliberate balancing across diverse ethnic and demographic backgrounds to ensure broad coverage.

Result: Created dataset of 3,534 stereotypes in English and 3,206 stereotypes across 15 native languages, establishing a reproducible methodology sensitive to the region's linguistic diversity and traditional orality.

Conclusion: The work demonstrates a scalable, culturally-sensitive approach to expanding stereotype resources for underrepresented regions, prioritizing targeted expansion over mere data volume increase for more comprehensive AI safety evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs represent concepts abstractly (independent of input format) by examining Function Vectors (FVs) used for in-context learning tasks and identifying more stable concept representations.

Method: Extract Function Vectors (FVs) from different input formats (open-ended vs. multiple-choice) targeting the same concept, identify Concept Vectors (CVs) using Representational Similarity Analysis (RSA) to select attention heads that encode concepts consistently across formats, and conduct steering experiments comparing FVs and CVs.

Result: FVs are not format-invariant - they are nearly orthogonal when extracted from different formats. CVs provide more stable concept representations. FV-related and CV-related heads emerge in similar layers but are largely distinct. FVs excel in-distribution when extraction and application formats match, while CVs generalize better out-of-distribution across question types and languages.

Conclusion: LLMs do contain abstract concept representations (CVs), but these differ from the representations that drive ICL performance (FVs). FVs are format-dependent while CVs provide more abstract, format-invariant concept representations with better cross-format generalization.
...
Method: Proposes a fusion architecture combining BanglaBERT-Large (for contextual understanding) with a two-layer stacked LSTM (for sequential dependencies). Fine-tuned and evaluated on a publicly available multilabel Bangla cyberbullying dataset covering cyberbully, sexual harassment, threat, and spam. Applied different sampling strategies to address class imbalance. Used 5-fold cross-validation to assess generalization.

Result: Evaluation uses multiple metrics: accuracy, precision, recall, F1-score, Hamming loss, Cohen's kappa, and AUC-ROC. The architecture is assessed for generalization through 5-fold cross-validation.

Conclusion: The proposed fusion architecture addresses limitations of standalone transformer and LSTM models by jointly modeling context and sequence for multilabel cyberbullying detection in Bangla, with comprehensive evaluation metrics and cross-validation to ensure robustness.
...
TL;DR: CultureManager is a pipeline for task-specific cultural alignment of LLMs that synthesizes task-aware cultural data from web searches and manages multi-culture knowledge with separate adapters and a culture router.
...
TL;DR: The paper introduces AI Sydney, a corpus of 4.5k LLM-generated texts (6M words) exploring human-AI relationships through three personas (Default, Classic Sydney, Memetic Sydney) simulated across 12 frontier models, with UD annotation and permissive licensing.


<details>
  <summary>Details</summary>
Motivation: To study how LLM-based entities conceive human-AI relationships, recognizing that both the model and the simulated personas matter, as demonstrated by the Sydney persona's cultural impact and memetic spread into training data of subsequent models.

Method: Created a corpus by simulating three author personas (Default Persona with no system prompt, Classic Sydney with original Bing system prompt, Memetic Sydney with "You are Sydney" prompt) across 12 frontier models from OpenAI, Anthropic, Alphabet, DeepSeek, and Meta, generating 4.5k texts totaling 6M words.

Result: Produced the AI Sydney corpus - 4.5k LLM-generated texts (6M words) annotated according to Universal Dependencies and available under a permissive license, enabling systematic study of human-AI relationship conceptions across different personas and models.

Conclusion: The AI Sydney corpus provides a valuable resource for analyzing how LLM personas conceive human-AI relationships, demonstrating the importance of both model architecture and simulated personas in understanding AI's cultural and safety implications.
